...
const useAccommodations = () => {
    ...
    const fetchAccommodations = useCallback(() => {
        accommodationRepository
            ...
            .then((response) => {
                setState({
                    "accommodations": Array.isArray(response.data) ? response.data : [],
                    "loading": false,
                });
            })
    ...
    const onEdit = useCallback((id, data) => {
        accommodationRepository
            .edit(id, data)
            .then(() => {
                console.log("Successfully edited a accommodation.");
                fetchAccommodations();
            })
            .catch((error) => console.log(error));
    }, [fetchAccommodations]);
    ...
    const onDelete = useCallback((id) => {
        accommodationRepository
            .delete(id)
            .then(() => {
                console.log("Successfully deleted a accommodation.");
                fetchAccommodations();
            })
            .catch((error) => console.log(error));
    }, [fetchAccommodations]);
